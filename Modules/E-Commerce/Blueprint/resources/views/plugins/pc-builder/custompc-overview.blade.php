...
<head>
    ...
    <title>{{ $storefrontName ?? config('app.name', 'Nexora Store') }} | {{ $product->name }}</title>
...
<body class="relative antialiased selection:bg-primary selection:text-white">
    ...
    @vite('Modules/E-Commerce/Blueprint/resources/js/Common/Preloader.js')
    ...
    <main class="flex-grow container mx-auto px-4 pt-32 pb-16 lg:pt-40 lg:pb-20 relative z-10 max-w-7xl">
        ...
        @php
            $cpu = $product->intelCpu ?? $product->amdCpu;
            $motherboard = $product->intelMotherboard ?? $product->amdMotherboard;
            $ram = $product->intelRam ?? $product->amdRam;
            $gpu = $product->gpu;
            $storage = $product->storage;
            $pcCase = $product->pcCase;
            $psu = $product->powerSupply;
            $cooler = $product->cooler;

            $rating = 5.0;
            $reviewCount = rand(5, 50);
            
            // Image logic (using case image as main image)
            $mainImg = $pcCase->image_url ?? 'https://images.unsplash.com/photo-1587202372634-32705e3bf49c?auto=format&fit=crop&w=800&q=80';
            
            $shortSpecs = [
                ['label' => 'Case', 'value' => $pcCase->name ?? 'Standard Case'],
                ['label' => 'Processor', 'value' => $cpu->name ?? 'N/A'],
                ['label' => 'Video Card', 'value' => $gpu->name ?? 'N/A'],
                ['label' => 'Memory', 'value' => $ram->name ?? 'N/A'],
                ['label' => 'Motherboard', 'value' => $motherboard->name ?? 'N/A'],
                ['label' => 'Storage', 'value' => $storage->name ?? 'N/A'],
            ...
            $detailedSpecs = [
                ...
                ['label' => 'Motherboard', 'value' => $motherboard->name ?? 'N/A'],
                ['label' => 'Storage', 'value' => $storage->name ?? 'N/A'],
                ['label' => 'Processor Cooling', 'value' => $cooler->name ?? 'Standard Cooler'],
                ['label' => 'Power Supply', 'value' => $psu->name ?? 'N/A'],
        ...
        <div class="flex flex-col lg:flex-row gap-12 mb-20">
            ...
            <div class="w-full lg:w-1/2 flex flex-col">
                ...
                <div class="flex items-center gap-2 mb-6">
                    ...
                    <span class="text-green-500 font-bold tracking-wider uppercase text-sm">Custom Build Selected</span>
                </div>
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        <div class="bg-[#050505]/50 border border-white/5 rounded-xl p-3 text-center backdrop-blur-sm group hover:border-primary/30 transition-colors cursor-default">
                            <div class="text-primary font-black text-xl mb-1">
                                {{ str_contains(strtoupper($gpu->name ?? ''), '4090') ? '160+' : (str_contains(strtoupper($gpu->name ?? ''), '4080') ? '140+' : '110+') }}
                            </div>
                            <div class="text-[9px] text-gray-400 font-bold uppercase">Cyberpunk</div>
                        </div>
                        <div class="bg-[#050505]/50 border border-white/5 rounded-xl p-3 text-center backdrop-blur-sm group hover:border-primary/30 transition-colors cursor-default">
                            <div class="text-primary font-black text-xl mb-1">
                                {{ str_contains(strtoupper($gpu->name ?? ''), '4090') ? '350+' : (str_contains(strtoupper($gpu->name ?? ''), '4080') ? '300+' : '220+') }}
                            </div>
                            <div class="text-[9px] text-gray-400 font-bold uppercase">Valorant</div>
                        </div>
                        <div class="bg-[#050505]/50 border border-white/5 rounded-xl p-3 text-center backdrop-blur-sm group hover:border-primary/30 transition-colors cursor-default">
                            <div class="text-primary font-black text-xl mb-1">
                                {{ str_contains(strtoupper($gpu->name ?? ''), '4090') ? '180+' : (str_contains(strtoupper($gpu->name ?? ''), '4080') ? '150+' : '120+') }}
                            </div>
