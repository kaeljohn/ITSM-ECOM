...
<head>
    ...
    <title>{{ $storefrontName ?? config('app.name', 'Nexora Store') }} | Accessories</title>
    ...
    @vite('Modules/E-Commerce/Blueprint/resources/css/liquidglass.css')
...
<body class="relative antialiased min-h-screen flex flex-col selection:bg-primary selection:text-white">
    ...
    <main class="flex-grow pt-32 lg:pt-40 pb-16">
        <div class="max-w-7xl mx-auto px-6 lg:px-8">
            ...
            <div class="mb-12 relative overflow-hidden rounded-2xl bg-gradient-to-r from-zinc-900 to-black border border-white/5 p-12">
                <div class="relative z-10">
                    <h1 class="text-4xl md:text-5xl font-black text-white tracking-wide mb-4">
                        Premium <span class="text-primary">PC Parts</span>
                    </h1>
                    <p class="text-gray-400 text-lg max-w-2xl">
                        Build your dream machine with our extensive collection of high-performance PC components. From processors to graphics cards, find exactly what you need.
                    </p>
                </div>
            ...
            <div class="flex flex-wrap items-center justify-center gap-3 mb-6" id="category-filters">
                <button data-filter="all" 
                   class="category-filter px-6 py-2 rounded-full border border-primary text-primary bg-primary/10 transition-colors text-sm font-semibold">
                    All
                </button>
                <button data-filter="cases" 
                   class="category-filter px-6 py-2 rounded-full border border-white/10 text-gray-400 hover:border-white/30 hover:text-white transition-colors text-sm font-semibold">
                    Cases
                </button>
                <button data-filter="chasis-fans" 
                   class="category-filter px-6 py-2 rounded-full border border-white/10 text-gray-400 hover:border-white/30 hover:text-white transition-colors text-sm font-semibold">
                    Chasis Fans
                </button>
                <button data-filter="storages" 
                   class="category-filter px-6 py-2 rounded-full border border-white/10 text-gray-400 hover:border-white/30 hover:text-white transition-colors text-sm font-semibold">
                    Storages
                </button>
                <button data-filter="video-cards" 
                   class="category-filter px-6 py-2 rounded-full border border-white/10 text-gray-400 hover:border-white/30 hover:text-white transition-colors text-sm font-semibold">
                    Video Cards
                </button>
                <button data-filter="processors" 
                   class="category-filter px-6 py-2 rounded-full border border-white/10 text-gray-400 hover:border-white/30 hover:text-white transition-colors text-sm font-semibold">
                    Processors
                </button>
                <button data-filter="coolers" 
                   class="category-filter px-6 py-2 rounded-full border border-white/10 text-gray-400 hover:border-white/30 hover:text-white transition-colors text-sm font-semibold">
                    Coolers
                </button>
                <button data-filter="motherboards" 
                   class="category-filter px-6 py-2 rounded-full border border-white/10 text-gray-400 hover:border-white/30 hover:text-white transition-colors text-sm font-semibold">
                    Motherboards
                </button>
                <button data-filter="rams" 
                   class="category-filter px-6 py-2 rounded-full border border-white/10 text-gray-400 hover:border-white/30 hover:text-white transition-colors text-sm font-semibold">
                    RAMs
                </button>
            </div>
            ...
            <div class="flex flex-col lg:flex-row gap-8 items-start">
                ...
                <div id="grid-fade-container" class="transition-opacity duration-300 ease-out opacity-100">
                    ...
                    <div id="no-products-message" class="hidden w-full py-12 flex flex-col items-center justify-center text-gray-400">
                        <i class="ph-light ph-magnifying-glass text-6xl mb-4 opacity-50"></i>
                        <p class="text-lg">No PC parts match your filters.</p>
                        <button onclick="document.getElementById('reset-filters-btn').click()" class="mt-4 text-primary hover:text-white transition-colors underline decoration-primary/30 underline-offset-4">Reset all filters</button>
                    </div>
                </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function updateUI(rebuildFilters = false) {
                ...
                setTimeout(() => {
                    ...
                    filterButtons.forEach(btn => {
                        ...
                        if (isActive) {
                            btn.className = 'category-filter px-6 py-2 rounded-full border border-primary text-primary bg-primary/10 transition-colors text-sm font-semibold';
                        } else {
                            btn.className = 'category-filter px-6 py-2 rounded-full border border-white/10 text-gray-400 hover:border-white/30 hover:text-white transition-colors text-sm font-semibold';
                        }
                    });
            ...
            function renderPagination(totalPages) {
                ...
                prevBtn.onclick = () => { if (currentPage > 1) { currentPage--; updateUI(); lenis.scrollTo('#category-filters', { offset: -150, duration: 1.2 }); } };
                ...
                for (let i = 1; i <= totalPages; i++) {
                    ...
                    pageBtn.onclick = () => { currentPage = i; updateUI(); lenis.scrollTo('#category-filters', { offset: -150, duration: 1.2 }); };
                ...
                nextBtn.onclick = () => { if (currentPage < totalPages) { currentPage++; updateUI(); lenis.scrollTo('#category-filters', { offset: -150, duration: 1.2 }); } };
