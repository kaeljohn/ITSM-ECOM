@php
    ...
    $layout = $storefrontCompany ? \Modules\Ecommerce\Models\StorefrontLayout::publishedFor($storefrontCompany) : [];
    ...
    $logoUrl = !empty($layout['logo_path']) ? (str_starts_with($layout['logo_path'], 'Modules/') ? Vite::asset($layout['logo_path']) : asset('storage/'.$layout['logo_path'])) : ($storefrontCompany?->logoUrl() ?: asset('ecommerce/Nexora_Logo.png'));
...
<head>
    ...
    <title>{{ $storefrontName }} | Gaming Laptops</title>
    ...
    <style>
        ...
        ::-webkit-scrollbar-thumb:hover {
            background: {{ $layout['primary_color'] ?? '#ff6b00' }};
        }
...
<body class="relative antialiased selection:bg-primary selection:text-white">
    ...
    <div id="preloader" class="fixed inset-0 bg-[#050505] z-[100] flex items-center justify-center transition-opacity duration-1000 ease-in-out">
        <script>
            if (!sessionStorage.getItem('{{ $storefrontVisitKey }}')) {
                document.write(`
    ...
    <main class="relative pt-32 pb-16 lg:pt-40 lg:pb-20 overflow-hidden w-full">
        <div class="w-full relative z-10 group" style="mask-image: linear-gradient(to right, transparent, black 15%, black 85%, transparent); -webkit-mask-image: linear-gradient(to right, transparent, black 15%, black 85%, transparent);">
            <div class="absolute inset-0 w-full h-full">
                <!-- Using a placeholder laptop image -->
                <img src="https://images.unsplash.com/photo-1593640408182-31c70c8268f5?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80" alt="Gaming Laptops" class="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-700 opacity-40">
            ...
            <div class="max-w-[1500px] mx-auto px-6 lg:px-8 relative z-10 py-16 md:py-24">
                <div class="w-full md:w-2/3">
                    <h1 class="text-4xl sm:text-5xl lg:text-6xl font-black text-white tracking-wide mb-4">Gaming Laptops</h1>
                    <p class="text-gray-400 text-sm md:text-base leading-relaxed max-w-lg">Discover high-performance gaming laptops for on-the-go gaming.</p>
                </div>
            </div>
        </div>
    </main>
    ...
    <form id="filter-form" method="GET" action="{{ route('ecommerce.gaming-laptops', ['store' => $store]) }}" class="max-w-[1500px] mx-auto px-6 lg:px-8 pb-24 relative z-10 flex flex-col lg:flex-row gap-8">
        ...
        <div id="product-grid-area" class="flex-1 w-full lg:w-auto transition-opacity duration-300">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                <p class="text-sm text-gray-400">Showing <span id="product-count" class="text-white font-bold">{{ $laptops->count() }}</span> products</p>
    ...
    <script>
        window.initialConfigs = @json($laptops);
        window.appUrl = "{{ url('/') }}";
            ...
            function applyFilters() {
                ...
                let filtered = configs.filter(product => {
                    ...
                    if (processors.length > 0) {
                        const procName = augmentProcName(product.processor);
                        if (!processors.includes(procName)) return false;
                    }

                    // GPU
                    if (gpus.length > 0) {
                        const gpuName = augmentGpuName(product.gpu);
                        if (!gpus.includes(gpuName)) return false;
                    }

                    // RAM
                    if (rams.length > 0) {
                        const ramName = product.ram;
    ...
    @vite(['Modules/E-Commerce/Blueprint/resources/js/Common/Preloader.js', 'Modules/E-Commerce/Blueprint/resources/js/Common/AmbientEffects.js'])
    @vite(['Modules/E-Commerce/Blueprint/resources/js/HomePage/Homepage.js', 'Modules/E-Commerce/Blueprint/resources/js/Category/Category.js'])
</body>
</html>
