        ...
        <tbody>
            @forelse ($recentListings as $listing)
                <tr>
                    ...
                    <td><span style="display:inline-block; padding: 2px 8px; border-radius: 12px; background: {{ $listing->status === 'active' ? 'rgba(22, 163, 74, 0.1)' : '#F4F6FA' }}; color: {{ $listing->status === 'active' ? '#16A34A' : '#5B7A9D' }}; font-size: 12px; font-weight: 500;">{{ $listing->status === 'active' ? 'Active' : 'Draft' }}</span></td>
                    <td>{{ $listing->available_quantity }}</td>
                    <td>&#8369;{{ number_format((float) ($listing->price ?: 0), 2) }}</td>
                </tr>
            @empty
