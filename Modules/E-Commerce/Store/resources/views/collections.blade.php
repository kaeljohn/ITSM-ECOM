@php
    ...
    $pageData = $pageData ?? collect($layout['custom_pages'] ?? [])->firstWhere('slug', 'collections') ?? collect($layout['custom_pages'] ?? [])->firstWhere('slug', 'gaming-laptops') ?? [];
    $title = $pageData['title'] ?? 'Collections';
    $subtitle = $pageData['subtitle'] ?? 'Browse our full range of high quality products. Experience uncompromised quality, ready to ship directly to your door.';
...
<body class="relative antialiased selection:bg-primary selection:text-white">
    ...
    <!-- Category Buttons -->
    @php
        $categoryButtons = $pageData['category_buttons'] ?? [];
        $hasButtons = collect($categoryButtons)->filter(fn($btn) => !empty($btn['label']))->isNotEmpty();
    @endphp
    @if($hasButtons || request('layout') == 'true')
    <div class="max-w-[1500px] mx-auto px-6 lg:px-8 relative z-10 py-8 flex flex-wrap gap-4 justify-center border-b border-white/5 mb-8" data-preview-section="collections-buttons">
        <ul id="collections-buttons-list" class="flex flex-wrap gap-4 justify-center w-full m-0 p-0 list-none">
            @if($hasButtons)
                @foreach($categoryButtons as $idx => $btn)
                    @if(!empty($btn['label']))
                    <li>
                        <a href="{{ $btn['url'] ?? '#' }}" data-parent-section="collections-buttons" data-preview-block="panel-collections-button-{{ $idx }}" class="px-8 py-3 rounded-full bg-white/5 border border-white/10 text-white hover:bg-white hover:text-black hover:border-white transition-all font-semibold shadow-lg inline-block">{{ $btn['label'] }}</a>
                    </li>
                    @endif
                @endforeach
            @elseif(request('layout') == 'true')
                <li><a href="#" data-parent-section="collections-buttons" data-preview-block="panel-collections-button-0" class="px-8 py-3 rounded-full bg-white/5 border border-white/10 text-white hover:bg-white hover:text-black hover:border-white transition-all font-semibold shadow-lg opacity-50 inline-block">Category 1</a></li>
