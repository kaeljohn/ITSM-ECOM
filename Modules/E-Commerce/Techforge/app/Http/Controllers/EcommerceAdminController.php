...
class EcommerceAdminController extends Controller
{
    ...
    public function saveLayout(Request $request): \Illuminate\Http\RedirectResponse|\Illuminate\Http\JsonResponse
    {
        ...
        $record = StorefrontLayout::query()->first() ?: StorefrontLayout::withoutGlobalScope('ecommerce-client')->first();
    ...
    public function previewLayout(Request $request)
    {
        ...
        if ($context && $context !== 'home') {
            ...
            switch ($blueprint) {
                ...
                case 'accessories':
                    return app(AccessoryController::class)->index($request);
                case 'store/monitors':
                case 'monitors':
                    return app(AccessoryController::class)->monitors($request);
                case 'store/pc-parts':
                case 'pc-parts':
                    return app(PcPartController::class)->index($request);
                case 'gaming-laptops':
                    return app(LaptopController::class)->index();
                case 'prebuilt-pcs':
                    return app(PrebuiltPcController::class)->index($request);
            }
        }
    ...
    public function publishLayout(Request $request): \Illuminate\Http\RedirectResponse|\Illuminate\Http\JsonResponse
    {
        ...
        try {
            ...
            return redirect()->route('ecommerce.admin.layout.edit')->withErrors(['layout' => $e->getMessage()]);
        }
    }
