...
<head>
    ...
    <title>{{ config('app.name', 'TechForge') }} | Gaming Laptops</title>
...
<body class="relative antialiased selection:bg-primary selection:text-white">
        ...
        <div class="w-full relative z-10 group" style="mask-image: linear-gradient(to right, transparent, black 15%, black 85%, transparent); -webkit-mask-image: linear-gradient(to right, transparent, black 15%, black 85%, transparent);">
            ...
            <div class="max-w-[1500px] mx-auto px-6 lg:px-8 relative z-10 py-16 md:py-24">
                <div class="w-full md:w-2/3">
                    <h1 class="text-4xl sm:text-5xl lg:text-6xl font-black text-white tracking-wide mb-4">{{ $pageData['title'] ?? 'Gaming Laptops' }}</h1>
                    <p class="text-gray-400 text-sm md:text-base leading-relaxed max-w-lg">{{ $pageData['subtitle'] ?? 'Discover high-performance gaming laptops for on-the-go gaming.' }}</p>
                </div>
            </div>
        </div>
    </main>
    ...
    @php
        try { $filterFormAction = route('ecommerce.gaming-laptops'); } catch (\Exception $e) { $filterFormAction = strtok(request()->url(), '?'); }
    @endphp
    <!-- Category Content -->
    <form id="filter-form" method="GET" action="{{ $filterFormAction }}" class="max-w-[1500px] mx-auto px-6 lg:px-8 pb-24 relative z-10 flex flex-col lg:flex-row gap-8">
