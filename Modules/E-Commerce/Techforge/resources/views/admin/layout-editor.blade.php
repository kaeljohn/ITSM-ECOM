...
<div class="editor-topbar">
    ...
    <div class="topbar-center">
        <select onchange="window.location.href = '?context=' + this.value">
            ...
            @php
                $storePageSlugs = ['store/accessories', 'store/monitors', 'store/pc-parts', 'gaming-laptops', 'prebuilt-pcs'];
                $storePages = collect($customPages)->filter(fn($p) => in_array($p['slug'], $storePageSlugs));
                $otherPages = collect($customPages)->reject(fn($p) => in_array($p['slug'], $storePageSlugs));
            @endphp
            
            @if($storePages->isNotEmpty())
            <optgroup label="Store Pages">
                @foreach($storePages as $cp)
            ...
            <div class="section-content">
                ...
                @php
                    $sLinks = $footer['shop_links'] ?? [
                        ...
                        ['label' => 'All Gaming Laptops', 'url' => '/gaming-laptops'],
...
<script>
    ...
    function addBlock(section, duplicateFromIdx = null) {
        ...
        if (hiddenInput && editorDiv) {
            ...
            hiddenInput.name = `${section}_blocks[${newIdx}][description]`;

            if (duplicateFromIdx !== null) {
                const originalInput = document.querySelector(`input[name="${section}_blocks[${duplicateFromIdx}][description]"]`);
                if (originalInput) {
    ...
    function removeBlock(section, idx) {
        if (idx < 4) {
            showToast('Minimum 4 item cards required.', 'error');
            return;
        }
        ...
        panels.forEach((panel, index) => {
            ...
            if (descInput) {
                descInput.name = `${section}_blocks[${index}][description]`;
            }
    ...
    (() => {
        ...
        const updateStaticPreview = () => {
            try {
                ...
                const syncSectionCards = (sectionId, containerSelector, itemPrefix, blockPanelClass) => {
                    const sectionEl = doc.querySelector(`section[data-preview-section="${sectionId}"]`);
                    if (!sectionEl) return;
                    ...
                    while (cards.length > blockPanels.length && cards.length > 4) {
                        const lastCard = cards[cards.length - 1];
                    ...
                    cards.forEach((card, idx) => {
                        const listingId = formData.get(`${sectionId}_blocks[${idx}][listing_id]`);
                        const descVal = formData.get(`${sectionId}_blocks[${idx}][description]`);
                ...
                syncSectionCards('tiers', '#tiers-carousel', 'panel-tiers-block-', 'tiers-block-panel');
                ...
                syncSectionCards('prebuilts', '#prebuilt-carousel', 'panel-prebuilts-block-', 'prebuilts-block-panel');
        ...
        window.submitPublishLive = async function() {
            ...
            try {
                const response = await fetch("{{ route('ecommerce.admin.layout.publish') }}", {
                    method: 'POST',
                    body: formData,
                    headers: {
