...
@php
    ...
    $resolveUrl = function(?string $url) use ($store) {
        $url = trim((string) $url);
        if (empty($url)) return '#';
        if (str_starts_with($url, '/') || str_starts_with($url, '#') || str_starts_with($url, 'http')) {
            return $url;
        }
        return url($url);
    };
    ...
    <footer class="border-t border-white/5 pt-16 pb-8 mt-auto relative z-10 liquid-glass bg-black/60 backdrop-blur-2xl">
        <div class="max-w-7xl mx-auto px-10 sm:px-12 lg:px-14">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 mb-12">
                ...
                <!-- Links 1 (Shop) -->
                <div>
                    <h4 id="footer-col1-title-el" class="text-primary font-black text-xs tracking-widest uppercase mb-6">{{ $col1Title }}</h4>
                    <ul id="footer-shop-links-list" class="space-y-4 text-[13px] text-gray-400 font-medium">
                        @foreach($shopLinks as $link)
                            @if(!empty($link['label']))
                            <li><a href="{{ $resolveUrl($link['url'] ?? '#') }}" class="hover:text-white transition-colors">{{ $link['label'] }}</a></li>
                            @endif
                        @endforeach
                    </ul>
                </div>
